"""Tests for is_excluded() — the per-scan query helper network_glm/lev1 uses.

Exact tuple match on (subject, session, task, run), considering only entries whose action
is in {"exclude", "trim"}. No entity normalization inside: the lockfile stores
BIDS-prefixed entities and lev1 queries with prefixed entities, so a bare query does NOT
match a prefixed entry. Normalization is the caller's job.
"""
...
def test_non_match_miss_different_session():
    # Same scan but ses-02 -> not in the list -> False.
...
def test_prefixed_vs_bare_no_normalization():
    """Faithful to the previous pipeline: is_excluded does NOT normalize entities.
...
def test_non_exclude_action_is_ignored():
    ...
    count as excluded, matching the previous pipeline's action filter."""
    fi_entries = [
